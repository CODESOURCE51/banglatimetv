...
$this->load->model('base_model');
	...
	$Data['News'] = $this->db->query('SELECT * FROM td_news')->num_rows();
	$Data['NewsSlider'] = $this->db->query('SELECT * FROM td_news WHERE slider=1')->num_rows();
	$Data['Video'] = $this->db->query('SELECT * FROM td_video')->num_rows();
	$Data['VideoHome'] = $this->db->query('SELECT * FROM td_video WHERE hmpage=1')->num_rows();
	$Data['album'] = $this->db->query('SELECT * FROM td_album')->num_rows();
	$Data['albumimg'] = $this->db->query('SELECT * FROM td_album_images')->num_rows();
	...
	public function multi_news()
	{
	if ( isset($_SESSION['user'])) {
		$this->load->view('selecting_news');
		}
		else {
		redirect(base_url().'admin.php');
		}
	...
	if ( isset($_SESSION['user'])) {
		...
		$this->load->view('news_list_delete',$category_news);
		}
		else {
		redirect(base_url().'admin.php');
		}
	}
	public function multi_news_delete($id){
		$ids = base64_decode(urldecode($id));
		$expid = explode('=',$ids);
		$exp = explode('%2C',$expid[0]);
		$imp = implode(',',$exp);
		
	$this->db->query('DELETE FROM td_news WHERE news_id IN ('.$imp.')');
	redirect(base_url().'admin.php/Dashboard/multi_news');
	}
	public function multi_videos_delete($id){
		$ids = base64_decode(urldecode($id));
		$expid = explode('=',$ids);
		$exp = explode('%2C',$expid[0]);
		$imp = implode(',',$exp);
		
	$this->db->query('DELETE FROM td_video WHERE vid IN ('.$imp.')');
	redirect(base_url().'admin.php/Dashboard/video_list');
	}
	...
	if ( isset($_SESSION['user'])) {
		...
		$category_news['post_list'] = $this->db->query('SELECT * FROM td_album ORDER BY aid DESC')->result_array();
		$this->load->view('album_list',$category_news);
		}
		else {
		redirect(base_url().'admin.php');
		}
	}
	public function album_list_delete()
	{
	if ( isset($_SESSION['user'])) {
		$cat_n = $this->input->post('txt_aCategory');
		
		$category_news['post_list'] = $this->db->query('SELECT * FROM td_album_images WHERE album_id="'.$cat_n.'" ORDER BY post_priority ASC')->result_array();
		$this->load->view('album_list_delete',$category_news);
		}
		else {
		redirect(base_url().'admin.php');
		}
	...
	$this->db->query('DELETE FROM td_album_images WHERE img_id IN ('.$imp.')');
	redirect(base_url().'admin.php/Dashboard/album_list');
	}
	...
	if ( isset($_SESSION['user'])) {
		...
		$news_inshort = $this->input->post('ninshrt');
	...
	{
	if ( isset($_SESSION['user'])) {
	$show['data'] = $this->db->query('SELECT MAX(sp_id) as mspid FROM td_sponsor')->result_array();
		$this->load->view('sponsor',$show);
	...
	{
	if ( isset($_SESSION['user'])) {
	$add_type = $this->input->post('inlineRadioOptionspp');
	$contents = $this->input->post('txt_content');
	$contents_title = $this->input->post('txt_content_title');
	...
	if ( isset($_SESSION['user'])) {
		...
		$showfor = $this->input->post('txt_home_show');
		$date = date('F').' '.date('d').','.date('Y');
		$fields = array(
			'video_cat' => $video_cat,
			'video' => $video,
	...
	if ( isset($_SESSION['user'])) {
	//$show['data'] = $this->base_model->show_data('td_cms_pages','*','')->result_array();
	$vdata['video'] = $this->db->query('SELECT td_video_album.cover_title,td_video.video,td_video.hmpage,td_video.date,td_video.vid FROM td_video_album INNER JOIN td_video ON td_video_album.v_album_id = td_video.video_cat')->result_array();
	...
	public function show_home($id,$vl)
	{
	if ( isset($_SESSION['user'])) {
	
	$this->db->query('UPDATE td_video SET hmpage='.$vl.' WHERE vid='.$id);
		redirect(base_url().'admin.php/Dashboard/view_video');
	...
	public function hide_home($id,$vl)
	{
	if ( isset($_SESSION['user'])) {
	$this->db->query('UPDATE td_video SET hmpage='.$vl.' WHERE vid='.$id);
		redirect(base_url().'admin.php/Dashboard/view_video');
